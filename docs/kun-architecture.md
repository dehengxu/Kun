...
`delegate_task` 把可信的内置和 GUI 配置目标统一成独立 agent profile 检索集合，
不再存在 skill worker。仓库可编辑的 `.kun/agents/*.md` 不进入自动召回，只能按
精确 ID 显式选择，并由宿主强制为仅本地读取（无网络/connector）、禁用 Skills、
屏蔽 model/provider/reasoning 覆盖。
